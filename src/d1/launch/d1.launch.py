...
def generate_launch_description():
    ...
    pkg_project = get_package_share_directory('d1')
    ...
    sdf_file = os.path.join(pkg_project, 'models', 'drone_model', 'model.sdf')
    with open(sdf_file, 'r') as infp:
            robot_desc = infp.read()
    ...
    rviz = Node(
       package='rviz2',
       executable='rviz2',
       arguments=['-d', os.path.join(pkg_project, 'config', 'd1.rviz')],
    ...
    bridge = Node(
        ...
        parameters=[{
            'config_file': os.path.join(pkg_project, 'config', 'bridge.yaml'),
